...
def _rule_based_explain_and_fix(issue: Dict[str, Any]) -> Dict[str, str]:
    ...
    return {
        "explanation": "Issue detected and enriched by the AI layer.",
...
def _build_score_explanation(
    ...
    low = int(by_severity.get("low") or 0)
    medium = int(by_severity.get("medium") or 0)
    high = int(by_severity.get("high") or 0)

    density = (score or {}).get("density_per_kloc") or {}
    penalty_breakdown = (score or {}).get("penalty_breakdown") or {}
...
def generate_ai_outputs(
    ...
    for issue in unified_issues or []:
        ...
        base_details = _rule_based_explain_and_fix(issue)
    ...
    if llm_is_enabled():
        for item in top_risky:
            llm_details = generate_llm_issue_details(item)
            if not llm_details:
                continue

            if llm_details.get("explanation"):
                item["explanation"] = llm_details["explanation"]
            if llm_details.get("fix"):
                item["fix"] = llm_details["fix"]
            if llm_details.get("risk"):
                item["risk"] = llm_details["risk"]
            if llm_details.get("impact"):
                item["impact"] = llm_details["impact"]
            ...
            for enriched_item in enriched:
                if (
                    enriched_item.get("tool") == item.get("tool")
                    and enriched_item.get("rule_id") == item.get("rule_id")
                    and enriched_item.get("file") == item.get("file")
                    and enriched_item.get("line") == item.get("line")
                ):
    ...
    if int((totals.get("by_severity") or {}).get("high") or 0) > 0:
        headline += (
            ", including high-severity findings. Immediate remediation is "
            "recommended."
        )
    elif any(str(x.get("tool") or "").lower() == "bandit" for x in enriched):
        headline += (
            " with a mix of security and code-quality findings. Security "
            "hotspots should be handled first."
        )
    else:
    ...
    score_explanation = _build_score_explanation(metrics, score)
    ...
    if llm_is_enabled():
        ...
        if llm_summary:
            ...
            if llm_summary.get("security_overview"):
                security_overview = llm_summary["security_overview"]
            if llm_summary.get("quality_overview"):
                quality_overview = llm_summary["quality_overview"]
            if llm_summary.get("priority_action"):
                priority_action = llm_summary["priority_action"]
            if llm_summary.get("recommendations"):
                recommendations = llm_summary["recommendations"]
            if llm_summary.get("score_explanation"):
                score_explanation = llm_summary["score_explanation"]
    ...
    output = {
        ...
        "note": (
            "LLM-enhanced analysis generated with rule-based fallback."
            if llm_used
            else "Rule-based explanations generated without LLM fallback usage."
        ),
        "llm_used": llm_used,
    }

    ai_summary_path = scan_path / "ai" / "ai_summary.json"
    analysis_path = scan_path / "ai" / "analysis.json"

    _write_json(analysis_path, output)
    return _write_json(ai_summary_path, output)